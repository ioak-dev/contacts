...
const authorizeFromSession = () => {
  const accessToken = getSessionValue(`contacts-access_token`);
  const refreshToken = getSessionValue(`contacts-refresh_token`);
  if (accessToken && refreshToken) {
    httpPost(
      ...
      .then((response: any) => {
        if (response.status === 200) {
          ...
          if (response.data?.access_token) {
            ...
            setSessionValue(`contacts-access_token`, newAccessToken);
          }
      ...
      .catch((error: any) => {
        removeSessionValue(`contacts-access_token`);
        removeSessionValue(`contacts-refresh_token`);
        return false;
      });
  } else {
    return false;
